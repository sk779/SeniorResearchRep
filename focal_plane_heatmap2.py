...
while (i_loop < pt_num):
    try:
        x_prime = random.uniform(15,55)
        x_prime_old = x_prime
        y_prime = random.uniform(15,55)
        y_prime_old = y_prime
        x_prime, y_prime, plane_number = convert(x_prime,y_prime)
#        focal_coord(x_prime,y_prime)
        print x_prime, y_prime, plane_number
...
dict_sum = len(dict[(0,0)]) + len(dict[(0,1)]) + len(dict[(1,0)]) + len(dict[(1,1)])
print dict_sum
print len(dict[(0,0)])
print "0,0 Capture Ratio ",len(dict[(0,0)])/float(dict_sum)
print "1,0 Capture Ratio ",len(dict[(1,0)])/float(dict_sum)
print "0,1 Capture Ratio ",len(dict[(0,1)])/float(dict_sum)
print "1,1 Capture Ratio ",len(dict[(1,1)])/float(dict_sum)
