...
grid_lim = 20 # number of x/y bins in heatmap
size = 0.02015 # fiber size (1 for 5000)
pt_num = 300000 # number of points (galaxies) to test
hex_num = 12 # number of focal plane iterations across heatmap
graph_bound = 40 # range of graph
...
for i in range(0,len(x_list)):
    if (i % 10 == 0):
        print i   
    square_number = square_convert2(x_list[i],y_list[i],0,0,(graph_bound/grid_lim),grid_lim)
...
for i in range(0,len(all_pt_x)):
    if (i % 10 == 0):
        print i   
    square_number = square_convert2(all_pt_x[i],all_pt_y[i],0,0,(graph_bound/grid_lim),grid_lim)
...
plt.xlim(-5, graph_bound+5)
plt.ylim(-5, graph_bound+5)
cb = plt.colorbar()
